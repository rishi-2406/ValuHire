...
export default function InterviewLobby({
  ...
  useEffect(() => {
    if (videoRef.current && stream && cameraWorking) {
      videoRef.current.srcObject = stream;
    }
  }, [stream, cameraWorking]);
        ...
        <div className="bg-white border border-outline-variant rounded-xl p-6 shadow-sm flex flex-col gap-4 relative overflow-hidden">
          ...
          <div className="space-y-2">
            ...
            <p className="text-base font-semibold text-[#111C2D]">
              {activeInterview.role || activeInterview.position || "Candidate Role"}
            </p>
          </div>
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-base text-[#111C2D] font-medium">
                {activeInterview.candidateName || activeInterview.name || "Candidate"}
              </p>
            </div>
          </div>
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-[#F0F3FF] text-[#111C2D] flex items-center justify-center font-bold text-sm">
                {(activeInterview.interviewerName || "Interviewer").substring(0, 1)}
              </div>
              <p className="text-base text-[#111C2D] font-medium">
                {activeInterview.interviewerName || "Interviewer"}
              </p>
            </div>
          </div>
          <button className="mt-2 w-full py-2 px-4 border border-primary text-primary font-medium text-sm rounded-lg hover:bg-primary/5 transition-colors">
            View Details
          </button>
        </div>
