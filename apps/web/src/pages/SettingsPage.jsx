...
import { User, Bell, Lock, Shield, Settings2, Settings as SettingsIcon, X, Plus } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [email] = useState(user?.email || "sarah.chen@valuhire.com");
  const [bio, setBio] = useState(user?.bio || "Senior Technical Recruiter with 8 years of experience finding top-tier engineering talent in the Bay Area. Passionate about diversity in tech.");
  const [specialties, setSpecialties] = useState(user?.specialties || ["Engineering", "Executive Search", "SaaS"]);
    ...
    <div className="app-shell bg-[#F8FAFC]">
      ...
      <main className="workspace">
        <TopBar
          ...
          actions={
            <div className="flex gap-3">
              ...
              <button type="submit" form="settings-form" className="bg-[#059669] hover:bg-[#047857] text-white px-5 py-2 rounded-lg font-bold text-sm transition-colors shadow-sm">
                Save Changes
              </button>
            </div>
          }
        ...
        <div className="p-8 max-w-5xl mx-auto h-full flex flex-col items-center justify-center">
          ...
          {/* Main Settings Card matching the design exactly */}
            ...
            <section className="flex-1 p-10 overflow-y-auto">
              {activeSection === "profile" && (
                <form id="settings-form" onSubmit={handleSave} className="max-w-2xl">
                  <div className="mb-8">
                    ...
                    <p className="text-on-surface-variant">Update your photo and personal details here.</p>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="relative">
                       <input 
                         ...
                         value={email} 
                         disabled
                         className="w-full border border-outline-variant/80 rounded pl-10 pr-4 py-2.5 text-sm bg-white focus:outline-none text-on-surface font-medium"
                       />
                  ...
                  <div className="mt-8 flex justify-end gap-3 border-t border-outline-variant/50 pt-6">
                    ...
                    <button type="submit" className="bg-[#41765B] hover:bg-[#2F5A44] text-white px-6 py-2.5 rounded font-bold text-sm transition-colors shadow-sm">
                      Save Changes
                    </button>
                  </div>
                </form>
              ...
              {activeSection !== "profile" && (
                <div className="flex flex-col items-center justify-center h-full text-on-surface-variant">
                  ...
                  <p className="text-sm mt-2">This section is currently under construction.</p>
                </div>
              )}
            </section>
          </div>
